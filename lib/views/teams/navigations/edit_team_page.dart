...
class EditTeamPage extends StatefulWidget {
  const EditTeamPage({Key? key}) : super(key: key);
...
class _EditTeamPageState extends State<EditTeamPage> {
  ...
  Widget build(BuildContext context) {
    ...
    playerPositionList = generalController.generalModel?.elements!.map((info) => info.elementType).toList();
    ...
    playerTeam = generalController.generalModel?.elements!.map((info) => info.team).toList();
    playerStatus = generalController.generalModel?.elements!.map((info) => info.status).toList();


    final positionMap = Map.fromIterables(idList??[], playerPositionList??[]);
    ...
    return Container(
        ...
        child: Scaffold(
            ...
            appBar: const CustomAppBar(title: "Edit Team",),
            body: Column(
              children: [
                Padding(
                  ...
                  child: Container(
                      ...
                      child: Padding(
                        ...
                        child: Text("Balance: ${num.parse(team[1].toStringAsFixed(2))}m",
                ...
                Expanded(
                  child: ListView.builder(
                      ...
                      itemBuilder: (ctx, index){
                        if(selectedPlayerPosition[index] == selectedPlayers[0]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[1]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[2]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[3]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[4]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[5]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[6]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[7]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[8]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[9]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[10]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[11]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[12]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[13]){
                          isPlayerSelected = true;
                        }else if(selectedPlayerPosition[index] == selectedPlayers[14]){
                          isPlayerSelected = true;
                        }else{
  ...
  showConfirmDialogue(String playerName, double price, int playerId) {
    return showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Confirm', style: TextStyle(color: Colors.black),),
        content: Text('Replace with ${utf8.decode(playerName.runes.toList())} for ${price}m?', style: const TextStyle(color: Colors.black),),
        actions:[
          ElevatedButton(
